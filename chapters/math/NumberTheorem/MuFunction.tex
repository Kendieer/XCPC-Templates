...
对于数论函数 $f,g$，常用莫比乌斯反演的式子有以下三种情况：

\begin{enumerate}
    \item $$f(n)=\sum_{d|n} g(d) \iff g(n) = \sum_{d|n} \mu(\frac{n}{d})\cdot f(d)$$
    \item $$f(n)=\sum_{n|d} g(d) \iff g(n) = \sum_{n|d} \mu(\frac{d}{n})\cdot f(d)$$
    \item $$f(n)=\prod_{d|n}g(d) \iff g(n) = \prod_{d|n} f(d)^{\mu(\frac{n}{d})}$$
\end{enumerate}